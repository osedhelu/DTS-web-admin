...
interface RouteContext {
  params: Promise<{ storeId: string; categoryId: string; imageId: string }>;
}
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  const { storeId, categoryId, imageId } = await context.params;
  const formData = await request.formData();

  try {
    const response = await fetch(
      `${baseUrl}/stores/${storeId}/categories/${categoryId}/images/${imageId}/`,
      {
        method: "PATCH",
